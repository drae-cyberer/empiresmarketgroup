        ...
        try {
            switch ($action) {
                case 'approve':
                    ...
                    // Get deposit details
                    $deposit = $db->selectOne("SELECT * FROM deposits WHERE id = ?", [$deposit_id]);
                    ...
                    if ($deposit && $deposit['status'] == 'PENDING') {
                        ...
                        $db->update("UPDATE deposits SET status = 'APPROVED', admin_note = ?, updated_at = NOW() WHERE id = ?", 
                                   [$admin_note, $deposit_id]);
                        
                        // Update user balance
                        $db->update("UPDATE users SET balance = balance + ?, total_deposits = total_deposits + ? WHERE id = ?", 
                                   [$deposit['amount'], $deposit['amount'], $deposit['user_id']]);
                        
                        // Add transaction record
                        $db->insert("INSERT INTO transactions (user_id, transaction_id, type, amount, description, status, created_at) VALUES (?, ?, 'DEPOSIT', ?, ?, 'COMPLETED', NOW())", 
                                   [$deposit['user_id'], $deposit['transaction_id'], $deposit['amount'], 'Deposit approved by admin']);
                        
                        $db->commit();
                        $message = 'Deposit approved successfully.';
                        $message_type = 'success';
                    } else {
                        $message = 'Invalid deposit or already processed.';
                        $message_type = 'error';
                    }
                    break;
                ...
                case 'reject':
                    $admin_note = $_POST['admin_note'] ?? '';
                    $db->update("UPDATE deposits SET status = 'REJECTED', admin_note = ?, updated_at = NOW() WHERE id = ?", 
                               [$admin_note, $deposit_id]);
...
if ($search) {
    $where_conditions[] = "(u.username LIKE ? OR d.transaction_id LIKE ?)";
    $search_param = "%$search%";
    $params[] = $search_param;
    $params[] = $search_param;
}
...
try {
    // Get total count
    $total_deposits = $db->count("SELECT COUNT(*) FROM deposits d JOIN users u ON d.user_id = u.id $where_clause", $params);
    ...
    $error_message = 'Error loading deposits: ' . $e->getMessage();
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Deposits Management - <?php echo SITE_NAME; ?></title>
    ...
    <style>
        ...
        * {
        ...
        .deposit-info .user-email {
            color: #64748b;
            font-size: 12px;
            margin-bottom: 4px;
        }

        .deposit-info .transaction-id {
            color: #94a3b8;
            font-size: 11px;
        }
...
<body>
        ...
        <aside class="admin-sidebar" id="adminSidebar">
            <nav class="admin-nav">
                <ul>
                    ...
                    <li><a href="deposits.php" class="active"><i class="fas fa-arrow-down"></i> Deposits</a></li>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($deposit['proof_image']): ?>
                                                <a href="../assets/images/uploads/<?php echo htmlspecialchars($deposit['proof_image']); ?>" 
                                        ...
                                        <td>
                                            <?php if ($deposit['status'] == 'PENDING'): ?>
                                                <div class="action-buttons">
                                                    <button class="btn btn-sm btn-success" 
                                                            onclick="processDeposit(<?php echo $deposit['id']; ?>, 'approve')" 
                                                            title="Approve">
                                                        <i class="fas fa-check"></i>
                                                    </button>
                                                    <button class="btn btn-sm btn-danger" 
                                                            onclick="processDeposit(<?php echo $deposit['id']; ?>, 'reject')" 
                                                            title="Reject">
                                            ...
                                            <?php else: ?>
                                                ...
                                                <?php if ($deposit['admin_note']): ?>
                                                    <br><small title="<?php echo htmlspecialchars($deposit['admin_note']); ?>">
                            ...
                            <div class="deposit-card">
                                <div class="deposit-card-header">
                                    <div class="deposit-info">
                                        <h4><?php echo htmlspecialchars($deposit['username']); ?></h4>
                                        <div class="user-email"><?php echo htmlspecialchars($deposit['email']); ?></div>
                                        <div class="transaction-id">ID: <?php echo htmlspecialchars($deposit['transaction_id']); ?></div>
                                    </div>
                                    <span class="status-badge <?php echo strtolower($deposit['status']); ?>">
                                        <?php echo $deposit['status']; ?>
                                    </span>
                                ...
                                <div class="deposit-details">
                                    <div class="detail-item">
                                        ...
                                        <div class="detail-value"><?php echo format_currency($deposit['amount']); ?></div>
                                    </div>
                                    <div class="detail-item">
                                        <div class="detail-label">Type</div>
                                        <div class="detail-value"><?php echo htmlspecialchars($deposit['deposit_type']); ?></div>
                                    </div>
                                    <div class="detail-item">
                                        <div class="detail-label">Date</div>
                                        <div class="detail-value"><?php echo date('M j, Y', strtotime($deposit['created_at'])); ?></div>
                                    </div>
                                    <div class="detail-item">
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="admin-pagination">
                        ...
                        <span class="pagination-info">
                            ...
                            (<?php echo number_format($total_deposits); ?> total deposits)
                        </span>
    ...
    <div id="processModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" id="processForm">
                ...
                <input type="hidden" name="deposit_id" id="processDepositId">
    ...
    <script>
        ...
        function processDeposit(depositId, action) {
            document.getElementById('processDepositId').value = depositId;
